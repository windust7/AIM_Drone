  ...
  if (f.SONAR_MODE) {
    if (calibratingS > 0) {
      if (!f.ARMED) { //init offset till motors not armed
        alt.EstAlt = alt.EstAlt * SONAR_BARO_LPF_LC + sonarAlt * (1 - SONAR_BARO_LPF_LC); // additional LPF to reduce baro noise (faster by 30 µs), comment was deleted by gp.zara
...
*/
    ...
    //debug[0] = conf.pid[PIDALT].P8;
    //debug[1] = conf.pid[PIDALT].I8;
    //debug[2] = conf.pid[PIDALT].D8;
    //P
    int16_t error16 = constrain(AltHold - alt.EstAlt, -300, 300);
    //applyDeadband(error16, 10); //remove small P parametr to reduce noise near zero position // commented by gp.zara
    ...
    //applyDeadband(accZ, ACC_Z_DEADBAND); // commented by gp.zara
    ...
    // applyDeadband(baroVel, 10); // to reduce noise near zero // commented by gp.zara
    ...
    //D
    alt.vario = vel;
    // applyDeadband(alt.vario, 5); // commented by gp.zara

...
for f in img_files:
    ...
    net.setInput(blob)
    out = net.forward()  # out.shape=(1, 57, 46, 46) -> 사람이 한명 있다고 생각하고 57개중에 앞에 18개(nparts)만 사용
    ...
    for i in range(nparts):
        heatMap = out[0, i, :, :] #float32값이 들어가있는 46x46 행렬
        ...
        _, conf, _, point = cv2.minMaxLoc(heatMap) #최대값 conf, 최대값크기 point
        x = int(w * point[0] / out.shape[3]) #입력영상 크기 기준

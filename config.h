...
/*************************************************************************************************/
  ...
  /******                Serial com speed    *********************************/
    ...
    #define SERIAL1_COM_SPEED 115200 // because of sensor board, 115200
...
/*************************************************************************************************/
  ...
  /************************        gameplusedu(GP) obs avoid by sonar         ********************/
  #define GP_OBS_AVOID_SONAR

  /************************        gameplusedu(GP) althold by sonar         ********************/
  #define GP_ALT_HOLD_SONAR
...
/*************************************************************************************************/
/////////////GP START//////////////
#if defined(GP_ALT_HOLD_SONAR) || defined(GP_OBS_AVOID_SONAR)
  ...
  */
  ...
  #define SONAR_MAX_HOLD 400          //cm, kind of error delimiter, for now to avoid rocket climbing, only usefull if no baro
  ...
  #if defined(GP_ALT_HOLD_SONAR) 
    
    #define SONAR_GENERIC_TRIGGER_PIN 4   // define sonar trig pin for alt hold 
    #define SONAR_GENERIC_ECHO_PIN 11   // define sonar echo pin for alt hold 
    ...
    // define pin, pinmode, toggle, pcint, pcint_vector
    ...
    #define SONAR_GEP_TriggerPin_PIN_HIGH    PORTG |= 1<<5; // PG5
    #define SONAR_GEP_TriggerPin_PIN_LOW     PORTG &= ~(1<<5);  // PG5
    #define SONAR_GEP_EchoPin                SONAR_GENERIC_ECHO_PIN
    #define SONAR_GEP_EchoPin_PINMODE_IN     pinMode(SONAR_GEP_EchoPin,INPUT);
    #define SONAR_GEP_EchoPin_PCINT          PCINT5 // was : PCINT5
    #define SONAR_GEP_EchoPin_PCICR          PCICR |= (1<<PCIE0); // PCINT 0-7 belong to PCIE0
    #define SONAR_GEP_EchoPin_PCMSK          PCMSK0 |= (1<<SONAR_GEP_EchoPin_PCINT); // set PCINT to trigger an interrupt on state change
  ...
  #if defined(GP_OBS_AVOID_SONAR)
    #define SONAR_GENERIC_FRONT_TRIGGER_PIN 49    // define sonar trig pin for front obs detecting   
    #define SONAR_GENERIC_FRONT_ECHO_PIN 12     // define sonar echo pin for front obs detecting
    #define SONAR_GENERIC_BACK_TRIGGER_PIN 47   // define sonar trig pin for back obs detecting  
    #define SONAR_GENERIC_BACK_ECHO_PIN 53      // define sonar echo pin for back obs detecting  
    #define SONAR_GENERIC_LEFT_TRIGGER_PIN 41   // define sonar trig pin for left obs detecting 
    #define SONAR_GENERIC_LEFT_ECHO_PIN 51    // define sonar echo pin for left obs detecting
    #define SONAR_GENERIC_RIGHT_TRIGGER_PIN 43    // define sonar trig pin for right obs detecting   
    #define SONAR_GENERIC_RIGHT_ECHO_PIN 52   // define sonar echo pin for right obs detecting

    // define pin, pinmode, toggle, pcint, pcint_vector
    /////////////FRONT///////////
    #define SONAR_GEP_FRONT_TriggerPin             SONAR_GENERIC_FRONT_TRIGGER_PIN
    #define SONAR_GEP_FRONT_TriggerPin_PINMODE_OUT pinMode(SONAR_GEP_FRONT_TriggerPin,OUTPUT);
    #define SONAR_GEP_FRONT_TriggerPin_PIN_HIGH    PORTL |= 1<<0; // PL0
    #define SONAR_GEP_FRONT_TriggerPin_PIN_LOW     PORTL &= ~(1<<0); // PL0
    #define SONAR_GEP_FRONT_EchoPin                SONAR_GENERIC_FRONT_ECHO_PIN
    #define SONAR_GEP_FRONT_EchoPin_PINMODE_IN     pinMode(SONAR_GEP_FRONT_EchoPin,INPUT);
    #define SONAR_GEP_FRONT_EchoPin_PCINT          PCINT6 // was PCINT6
    #define SONAR_GEP_FRONT_EchoPin_PCICR          PCICR |= (1<<PCIE0); // PCINT 0-7 belong to PCIE0
    #define SONAR_GEP_FRONT_EchoPin_PCMSK          PCMSK0 |= (1<<SONAR_GEP_FRONT_EchoPin_PCINT); // Mask Pin PCINT5 - all other PIns PCINT0-7 are not allowed to create interrupts!
    #define SONAR_GEP_FRONT_EchoPin_PCINT_vect     PCINT0_vect  // PCINT0-7 belog PCINT0_vect
    #define SONAR_GEP_FRONT_EchoPin_PIN            PINB  // PCINT0-7 belong to PINB
    
    /////////////BACK///////////
    #define SONAR_GEP_BACK_TriggerPin             SONAR_GENERIC_BACK_TRIGGER_PIN
    #define SONAR_GEP_BACK_TriggerPin_PINMODE_OUT pinMode(SONAR_GEP_BACK_TriggerPin,OUTPUT);
    #define SONAR_GEP_BACK_TriggerPin_PIN_HIGH    PORTL |= 1<<2;  // PL2
    #define SONAR_GEP_BACK_TriggerPin_PIN_LOW     PORTL &= ~(1<<2); // PL2
    #define SONAR_GEP_BACK_EchoPin                SONAR_GENERIC_BACK_ECHO_PIN
    #define SONAR_GEP_BACK_EchoPin_PINMODE_IN     pinMode(SONAR_GEP_BACK_EchoPin,INPUT);
    #define SONAR_GEP_BACK_EchoPin_PCINT          PCINT0
    #define SONAR_GEP_BACK_EchoPin_PCICR          PCICR |= (1<<PCIE0); // PCINT 0-7 belong to PCIE0
    #define SONAR_GEP_BACK_EchoPin_PCMSK          PCMSK0 |= (1<<SONAR_GEP_BACK_EchoPin_PCINT); // Mask Pin PCINT5 - all other PIns PCINT0-7 are not allowed to create interrupts!
    #define SONAR_GEP_BACK_EchoPin_PCINT_vect     PCINT0_vect  // PCINT0-7 belog PCINT0_vect
    #define SONAR_GEP_BACK_EchoPin_PIN            PINB  // PCINT0-7 belong to PINB
    
    /////////////LEFT///////////
    #define SONAR_GEP_LEFT_TriggerPin             SONAR_GENERIC_LEFT_TRIGGER_PIN
    #define SONAR_GEP_LEFT_TriggerPin_PINMODE_OUT pinMode(SONAR_GEP_LEFT_TriggerPin,OUTPUT);
    #define SONAR_GEP_LEFT_TriggerPin_PIN_HIGH    PORTG |= 1<<0;  //PG0
    #define SONAR_GEP_LEFT_TriggerPin_PIN_LOW     PORTG &= ~(1<<0); //PG0
    #define SONAR_GEP_LEFT_EchoPin                SONAR_GENERIC_LEFT_ECHO_PIN
    #define SONAR_GEP_LEFT_EchoPin_PINMODE_IN     pinMode(SONAR_GEP_LEFT_EchoPin,INPUT);
    #define SONAR_GEP_LEFT_EchoPin_PCINT          PCINT2
    #define SONAR_GEP_LEFT_EchoPin_PCICR          PCICR |= (1<<PCIE0); // PCINT 0-7 belong to PCIE0
    #define SONAR_GEP_LEFT_EchoPin_PCMSK          PCMSK0 |= (1<<SONAR_GEP_LEFT_EchoPin_PCINT); // Mask Pin PCINT5 - all other PIns PCINT0-7 are not allowed to create interrupts!
    #define SONAR_GEP_LEFT_EchoPin_PCINT_vect     PCINT0_vect  // PCINT0-7 belog PCINT0_vect
    #define SONAR_GEP_LEFT_EchoPin_PIN            PINB  // PCINT0-7 belong to PINB
    
    /////////////RIGHT///////////
    #define SONAR_GEP_RIGHT_TriggerPin             SONAR_GENERIC_RIGHT_TRIGGER_PIN
    #define SONAR_GEP_RIGHT_TriggerPin_PINMODE_OUT pinMode(SONAR_GEP_RIGHT_TriggerPin,OUTPUT);
    #define SONAR_GEP_RIGHT_TriggerPin_PIN_HIGH    PORTL |= 1<<6; // PL6
    #define SONAR_GEP_RIGHT_TriggerPin_PIN_LOW     PORTL &= ~(1<<6);  // PL6
    #define SONAR_GEP_RIGHT_EchoPin                SONAR_GENERIC_RIGHT_ECHO_PIN
    #define SONAR_GEP_RIGHT_EchoPin_PINMODE_IN     pinMode(SONAR_GEP_RIGHT_EchoPin,INPUT);
    #define SONAR_GEP_RIGHT_EchoPin_PCINT          PCINT1
    #define SONAR_GEP_RIGHT_EchoPin_PCICR          PCICR |= (1<<PCIE0); // PCINT 0-7 belong to PCIE0
    #define SONAR_GEP_RIGHT_EchoPin_PCMSK          PCMSK0 |= (1<<SONAR_GEP_RIGHT_EchoPin_PCINT); // Mask Pin PCINT5 - all other PIns PCINT0-7 are not allowed to create interrupts!
    #define SONAR_GEP_RIGHT_EchoPin_PCINT_vect     PCINT0_vect  // PCINT0-7 belog PCINT0_vect
    #define SONAR_GEP_RIGHT_EchoPin_PIN            PINB  // PCINT0-7 belong to PINB
    
    #define GP_SONAR_MAX_DISTANCE   150     // define max distance we can use, cm

    // cm, if something is detected in 0 ~ this value, we avoid ( ~ 100 is recommended)
    #define GP_FRONT_SONAR_DISTANCE_OF_AVOID  100 //100
    #define GP_BACK_SONAR_DISTANCE_OF_AVOID  100 //100
    #define GP_LEFT_SONAR_DISTANCE_OF_AVOID  100 //100
    #define GP_RIGHT_SONAR_DISTANCE_OF_AVOID  100 //100

    // define the avoid power, 1500 means neutral (1200 ~ 1800 is recommended)
    #define GP_POWER_IF_FRONT_AND_BACK_OBS  1500
    #define GP_POWER_IF_FRONT_OBS 1200 // lower than other direction because we need more power on front.
    #define GP_POWER_IF_BACK_OBS  1700
    #define GP_POWER_IF_RIGHT_AND_LEFT_OBS  1500
    #define GP_POWER_IF_RIGHT_OBS 1300
    #define GP_POWER_IF_LEFT_OBS  1700

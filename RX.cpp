...
uint16_t readRawRC(uint8_t chan) {
  ...
  #endif
  ///////////////////////////////////// inserted by GP  start//
  #if defined(GP_OBS_AVOID_SONAR) // only when GP_OBS_AVOID_SONAR is defined, obs avoid sonar is possible
    if(f.SONAR_MODE) {
    // obs avoid by sonar & althold by sonar is activated when SONAR_MODE is on (1), let mode toggle by toggling aux switch
    
    int distance_front_cm = 0;
    int distance_right_cm= 0;
    int distance_back_cm= 0;
    int distance_left_cm= 0;
   
      ///////////pitch start//////////////////
      if(rcChannel[chan] == PITCHPIN) {
        distance_front_cm = sonarFrontDistance;   // sonarFrontDistance is regularly updated
        distance_back_cm = sonarBackDistance;     // sonarBackDistance is regularly updated
        //debug[0] = sonarFrontDistance;
        //debug[1] = sonarBackDistance;
        if(distance_front_cm > GP_SONAR_MAX_DISTANCE) { distance_front_cm = GP_SONAR_MAX_DISTANCE; }
        if(distance_back_cm > GP_SONAR_MAX_DISTANCE) { distance_back_cm = GP_SONAR_MAX_DISTANCE; }
        if(1 < distance_front_cm && distance_front_cm < GP_FRONT_SONAR_DISTANCE_OF_AVOID && 1 < distance_back_cm && distance_back_cm < GP_BACK_SONAR_DISTANCE_OF_AVOID) {
          // obs on front, and on back
          data = GP_POWER_IF_FRONT_AND_BACK_OBS;
        }
        else if(1 < distance_front_cm && distance_front_cm < GP_FRONT_SONAR_DISTANCE_OF_AVOID) {
          // obs only on front
          data = GP_POWER_IF_FRONT_OBS;
        }
        else if(1 < distance_back_cm && distance_back_cm < GP_BACK_SONAR_DISTANCE_OF_AVOID) {
          // obs only on back
          data = GP_POWER_IF_BACK_OBS;
        }      
      }
      ///////////pitch end//////////////////
  
      ///////////roll start//////////////////
      if(rcChannel[chan] == ROLLPIN) {
        distance_right_cm = sonarRightDistance;   // sonarRightDistance is regularly updated
        distance_left_cm = sonarLeftDistance;   // sonarLeftDistance is regularly updated
        debug[2] = sonarRightDistance;
        debug[3] = sonarLeftDistance;
        if(distance_right_cm > GP_SONAR_MAX_DISTANCE) { distance_right_cm = GP_SONAR_MAX_DISTANCE; }
        if(distance_left_cm > GP_SONAR_MAX_DISTANCE) { distance_left_cm = GP_SONAR_MAX_DISTANCE; }
        if(1 < distance_right_cm && distance_right_cm < GP_RIGHT_SONAR_DISTANCE_OF_AVOID && 1 < distance_left_cm && distance_left_cm < GP_LEFT_SONAR_DISTANCE_OF_AVOID) {
          // obs on right, and on left
          data = GP_POWER_IF_RIGHT_AND_LEFT_OBS;
        }
        else if(1 < distance_right_cm && distance_right_cm < GP_RIGHT_SONAR_DISTANCE_OF_AVOID) {
          // obs only on right
          data = GP_POWER_IF_RIGHT_OBS;
        }
        else if(1 < distance_left_cm && distance_left_cm < GP_LEFT_SONAR_DISTANCE_OF_AVOID) {
          // obs only on left
          data = GP_POWER_IF_LEFT_OBS;
        }      
      }
      ///////////roll end//////////////////  
    } 

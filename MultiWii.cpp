...
int16_t  sonarAlt;
int16_t  sonarFrontDistance;
int16_t  sonarFrontDistanceMission;
int16_t  sonarBackDistance;
int16_t  sonarLeftDistance;
int16_t  sonarRightDistance;
int16_t  BaroPID = 0;
...
void loop () {
  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    if (rcOptions[BOXSONAR]) {
      ...
      #else
              initialThrottleHold = rcCommand[THROTTLE]; //////////////////////////here!!!!!!!!!!!!!!!!!!!gp.zara
      #endif
  ...
  #if (BARO && (!defined(SUPPRESS_BARO_ALTHOLD))) || SONAR // edited by gp.zara 20170623
  //#if BARO && (!defined(SUPPRESS_BARO_ALTHOLD))
  ...
  */
  if (f.BARO_MODE || f.SONAR_MODE) { // edited by gp.zara 20170623
  //if (f.BARO_MODE) {

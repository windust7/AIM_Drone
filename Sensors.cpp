...
volatile unsigned long SONAR_GEP_Front_startTime = 0;
volatile unsigned long SONAR_GEP_Back_startTime = 0;
volatile unsigned long SONAR_GEP_Left_startTime = 0;
volatile unsigned long SONAR_GEP_Right_startTime = 0;

volatile unsigned long SONAR_GEP_echoTime = 0;
volatile unsigned long SONAR_GEP_Front_echoTime = 0;
volatile unsigned long SONAR_GEP_Back_echoTime = 0;
volatile unsigned long SONAR_GEP_Left_echoTime = 0;
volatile unsigned long SONAR_GEP_Right_echoTime = 0;

volatile static int32_t  tempSonarAlt = 0;
volatile static int32_t  tempSonarFrontDistance = 0;
volatile static int32_t  tempSonarBackDistance = 0;
volatile static int32_t  tempSonarLeftDistance = 0;
volatile static int32_t  tempSonarRightDistance = 0;
...
void Sonar_init() {
  ...
  SONAR_GEP_FRONT_EchoPin_PCICR;
  SONAR_GEP_FRONT_EchoPin_PCMSK;
  SONAR_GEP_FRONT_EchoPin_PINMODE_IN;
  SONAR_GEP_FRONT_TriggerPin_PINMODE_OUT;

  SONAR_GEP_BACK_EchoPin_PCICR;
  SONAR_GEP_BACK_EchoPin_PCMSK;
  SONAR_GEP_BACK_EchoPin_PINMODE_IN;
  SONAR_GEP_BACK_TriggerPin_PINMODE_OUT;

  SONAR_GEP_LEFT_EchoPin_PCICR;
  SONAR_GEP_LEFT_EchoPin_PCMSK;
  SONAR_GEP_LEFT_EchoPin_PINMODE_IN;
  SONAR_GEP_LEFT_TriggerPin_PINMODE_OUT;

  SONAR_GEP_RIGHT_EchoPin_PCICR;
  SONAR_GEP_RIGHT_EchoPin_PCMSK;
  SONAR_GEP_RIGHT_EchoPin_PINMODE_IN;
  SONAR_GEP_RIGHT_TriggerPin_PINMODE_OUT;
}

void Sonar_update() {
  //noInterrupts();
  /*
  sonarAlt = 1 + tempSonarAlt; // cm
  ...
  */
  ...
  if(f.SONAR_MODE) {
    sonarFrontDistance = 1 + tempSonarFrontDistance;
    SONAR_GEP_FRONT_TriggerPin_PIN_LOW;
    delayMicroseconds(2);
    SONAR_GEP_FRONT_TriggerPin_PIN_HIGH;
    delayMicroseconds(10);
    SONAR_GEP_FRONT_TriggerPin_PIN_LOW;
  
    sonarBackDistance = 1 + tempSonarBackDistance;
    SONAR_GEP_BACK_TriggerPin_PIN_LOW;
    delayMicroseconds(2);
    SONAR_GEP_BACK_TriggerPin_PIN_HIGH;
    delayMicroseconds(10);
    SONAR_GEP_BACK_TriggerPin_PIN_LOW;
  
    sonarLeftDistance = 1 + tempSonarLeftDistance;
    SONAR_GEP_LEFT_TriggerPin_PIN_LOW;
    delayMicroseconds(2);
    SONAR_GEP_LEFT_TriggerPin_PIN_HIGH;
    delayMicroseconds(10);
    SONAR_GEP_LEFT_TriggerPin_PIN_LOW;
  
    sonarRightDistance = 1 + tempSonarRightDistance;
    SONAR_GEP_RIGHT_TriggerPin_PIN_LOW;
    delayMicroseconds(2);
    SONAR_GEP_RIGHT_TriggerPin_PIN_HIGH;
    delayMicroseconds(10);
    SONAR_GEP_RIGHT_TriggerPin_PIN_LOW;  
  }
  ...
  if(changedbits & (1 << PINB3))  // PCINT3 changed /
  {
    if(SONAR_GEP_EchoPin_PIN & (1 << SONAR_GEP_EchoPin_PCINT)) {
      ...
      if(SONAR_GEP_echoTime <= SONAR_GENERIC_MAX_RANGE*SONAR_GENERIC_SCALE) {
        ...
        //debug[1] = tempSonarAlt;  
      }
      else {
        tempSonarAlt = SONAR_MAX_HOLD;  
        //tempSonarAlt = -1; 
        //debug[2] = tempSonarAlt;  
      }
    }
  }
  */
  ...
  if(f.SONAR_MODE) {
    if(changedbits & (1 << PINB6))  // PCINT6 changed //
    {
      if(SONAR_GEP_FRONT_EchoPin_PIN & (1 << SONAR_GEP_FRONT_EchoPin_PCINT)) {
        SONAR_GEP_Front_startTime = micros();
      }
      else {
        SONAR_GEP_Front_echoTime = micros() - SONAR_GEP_Front_startTime;
        if(SONAR_GEP_Front_echoTime <= SONAR_GENERIC_MAX_RANGE*SONAR_GENERIC_SCALE)
          tempSonarFrontDistance = SONAR_GEP_Front_echoTime / SONAR_GENERIC_SCALE;
        else
          tempSonarFrontDistance = -1;
      }  
    }
    else if(changedbits & (1 << PINB0))  // PCINT0 changed /
    {
      if(SONAR_GEP_BACK_EchoPin_PIN & (1 << SONAR_GEP_BACK_EchoPin_PCINT)) {
        SONAR_GEP_Back_startTime = micros();
      }
      else {
        SONAR_GEP_Back_echoTime = micros() - SONAR_GEP_Back_startTime;
        if(SONAR_GEP_Back_echoTime <= SONAR_GENERIC_MAX_RANGE*SONAR_GENERIC_SCALE)
          tempSonarBackDistance = SONAR_GEP_Back_echoTime / SONAR_GENERIC_SCALE;
        else
          tempSonarBackDistance = -1;
      }  
    }
    else if(changedbits & (1 << PINB2))  // PCINT2 changed */
    {
      if(SONAR_GEP_LEFT_EchoPin_PIN & (1 << SONAR_GEP_LEFT_EchoPin_PCINT)) {
        SONAR_GEP_Left_startTime = micros();
      }
      else {
        SONAR_GEP_Left_echoTime = micros() - SONAR_GEP_Left_startTime;
        if(SONAR_GEP_Left_echoTime <= SONAR_GENERIC_MAX_RANGE*SONAR_GENERIC_SCALE)
          tempSonarLeftDistance = SONAR_GEP_Left_echoTime / SONAR_GENERIC_SCALE;
        else
          tempSonarLeftDistance = -1;
      }  
    }
    //if(changedbits & (1 << PINB1))  // PCINT1 changed */
    else if(changedbits & (1 << PINB1))  /* PCINT1 changed */
    {
      if(SONAR_GEP_RIGHT_EchoPin_PIN & (1 << SONAR_GEP_RIGHT_EchoPin_PCINT)) {
        SONAR_GEP_Right_startTime = micros();
      }
      else {
        SONAR_GEP_Right_echoTime = micros() - SONAR_GEP_Right_startTime;
        if(SONAR_GEP_Right_echoTime <= SONAR_GENERIC_MAX_RANGE*SONAR_GENERIC_SCALE)
          tempSonarRightDistance = SONAR_GEP_Right_echoTime / SONAR_GENERIC_SCALE;
        else
          tempSonarRightDistance = -1;
      }  
    }  
  }
}
